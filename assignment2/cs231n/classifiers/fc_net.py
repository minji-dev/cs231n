...
class FullyConnectedNet(object):
    ...
    def __init__(
        ...
        for x in range(1, self.num_layers+1):
            if x == 1: input_dim_real = input_dim
            else: input_dim_real = hidden_dims[x-2]
                
            if x == self.num_layers: output_dim = num_classes
            else: output_dim = hidden_dims[x-1]
            
            self.params['W'+str(x)] = np.random.randn(input_dim_real, output_dim) * weight_scale
            self.params['b'+str(x)] = np.zeros(output_dim)
                
            if normalization == 'batchnorm' and x != self.num_layers:
                self.params['gamma'+str(x)] = np.ones_like(output_dim)
                self.params['beta'+str(x)] = np.zeros_like(output_dim)
    ...
    def loss(self, X, y=None):
        ...
        for i in range(1, self.num_layers): # 1~5
            keyW = 'W' + str(i)
            keyb = 'b' + str(i)
            if self.normalization != 'batchnorm':
                out, affine_relu_cache[i] = affine_relu_forward(out, self.params[keyW], self.params[keyb])
            else:
                key_gamma = 'gamma' + str(i)
                key_beta = 'beta' + str(i)
                out, affine_bn_relu_cache[i] = affine_bn_relu_forward(out, self.params[keyW], self.params[keyb], self.params[key_gamma], self.params[key_beta], self.bn_params[i-1])
        ...
        affine_out, affine_cache = affine_forward(out, self.params[keyW], self.params[keyb])
        scores = affine_out
