...
def affine_forward(x, w, b):
    ...
    D = np.prod(x.shape[1:])
    M = b.shape[0]
    out = np.dot(x.reshape(N, D), w.reshape(D, M)) + b
...
def softmax_loss(x, y):
    ...
    x -= np.max(x, axis=1, keepdims=True) # numeric instability

    x = np.exp(x) # exp 취함
    x_sum = x.sum(axis=1, keepdims=True) # 다 더한 거
    normalize = x/x_sum # 다 더한 걸로 나눠주기
    correct = normalize[np.arange(num_train), y]
    loss = -np.sum(np.log(correct)) / num_train

    # Grdient
    dx = np.zeros(x.shape)
    normalize[np.arange(num_train), y] -= 1 # 정답이면 해당 row번째 사진의 pixel value만큼 빼주기
    dx = normalize/num_train
...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == "train":
        ...
        cache = (x, sample_mean, sample_var, x_normal, gamma, beta, eps)
...
def layernorm_forward(x, gamma, beta, ln_param):
    ...
    cache = (x, feature_mean, feature_var, gamma, eps)
...
def layernorm_backward(dout, cache):
    ...
    N = dout.shape[1]
    x, feature_mean, feature_var, gamma, eps = cache
    x_normal = (x - feature_mean) / np.sqrt(feature_var + eps)
...
def dropout_forward(x, dropout_param):
    ...
    if mode == "train":
        ...
        # p는 dropout ratio. 살리는 비율임. 0.7이면 output 30%를 날린다.
        # output에서 (1-p)만큼 날리니 output의 전체 비율이 p로 줄어듦. 따라서 p로 나눠 기존 output과 같게 scale 해야 함.
        ...
        for f in range(F):
            for i in range(H_out):
                for j in range(W_out):
                    h_start = i * stride
                    w_start = j * stride
                    x_mask = x_pad[n, :, h_start:h_start+HH, w_start:w_start+WW]
...
def spatial_batchnorm_forward(x, gamma, beta, bn_param):
    ...
    # 기존 batch norm 함수를 x를 잠깐 flatten 해 적용하고, 다시 기존의 차원 모양으로 reshape
...
def spatial_groupnorm_backward(dout, cache):
    ...
    x, mean, var, gamma, G, eps = cache # mean, var: [N,G] x: [N,G,C//G,H,W]
    N, C, H, W = dout.shape
    ...
    dlmean += dlvar * np.sum(-2 * (x - mean), axis = (2,3,4), keepdims=True) / M
